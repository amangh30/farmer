...
const SignUp_Farmer = () => {
  const { access, setAccess } = useGlobal();
  ...
  return (
    <div>
      {access?(<Navigate to="/dashboard" replace={true} />):(<div className="bg3">
...
<div className='wrapper'>
  <form onSubmit={handleEvent} action="">
    ...
    <div className="input-box">
      <input style={{color:'white'}} type="text" placeholder='NAME' value={name} onChange={handleName} required />
      <PersonIcon style={{ fontSize: '35px' }} className='icon' />
    </div>


    <div className="input-box">
      <input style={{color:'white'}} type="text" placeholder='EMAIL-ID' value={email} onChange={handleEmail} required />
      <EmailIcon style={{ fontSize: '35px' }} className='icon' />
    </div>

    <div className="input-box">
      <input style={{color:'white'}} type="password" placeholder='PASSWORD' value={password} onChange={handlePass} required />
    ...
    <button onClick={handleEvent} type="submit">Sign-Up</button>

    <div className="register-link">
      <p>ALREADY HAVE AN ACCOUNT?  <Link to='/loginf'>LogIn</Link></p>
    </div>
